//
//  AceVirtualObject.swift
//  RemoteAssistance
//
//  Created by Yulius Tjahjadi on 1/20/20.
...
extension AceVirtualObject {
    // MARK: Static Properties and Methods
    /// Loads all the model objects within `Models.scnassets`.
    static let availableObjects: [AceVirtualObject] = {
...
//            guard url.pathExtension == "scn" && !url.path.contains("lighting") else { return nil }
    ...
    static func object(byName name:String) -> AceVirtualObject? {
    ...
    static func existingObjectContainingNode(_ node: SCNNode) -> AceVirtualObject? {
        if let virtualObjectRoot = node as? AceVirtualObject {
